...
INSERT INTO employees (employee_name, department_id, salary, hire_date) VALUES 
    ...
    -- op1: Inner Join to Retrieve Employee and Department Information

    select *  from employees  JOIN departments on employees.department_id = departments.department_id; 

    select *  from employees  JOIN departments USING(department_id); 

    -- op2: Group By Department with Average Salary
    ...
    -- op3: Count Employees in Each Department

    select department_name,  count(*) as total_emp from employees JOIN departments USING(department_id) GROUP BY department_name; 

    -- op4: Find the Department name with the Highest Average Salary

    select department_name , avg(salary) as highest_avgSalary from employees join departments USING (department_id) GROUP BY department_name ORDER BY highest_avgSalary DESC LIMIT 1;   

-- op5:Count Employees Hired Each Year
